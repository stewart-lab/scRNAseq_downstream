...
# set working dir
wkdir <- "/w5home/bmoore/Pierre_sc_zebrafish/cellchat2"
setwd(wkdir)
...
for (d in dirs) {
    if (str_detect(d, "seurat_mouse_annot_")) {
        ...
        name <- str_extract(filename, "\\d+WkN?R")
        print(name)
...
x <- c("source", "target", "condition", "timepoint", "pathway_name", "prob")
colnames(df_T) <- x
...
for (sender in senderList) {
    for (i in 1:length(data_listNR)) {
        ...
        data_subset <- data_subset[, c("source", "target", "pathway_name", "prob")]
        # get name (timepoint)
        name <- names(data_listNR)[i]
        timepoint <- str_split_i(name, "Wk", 1)
        condition <- str_split_i(name, "Wk", 2)
        ...
        long_df_sub$condition <- condition
...
# write
write.table(df_T,
    file = "Tcellsource_interactionstrengths_reps_OT.txt", sep = "\t",
    quote = F, row.names = F
)
write.table(df_T,
    file = "Fibroblastssource_interactionstrengths_reps_OT.txt", sep = "\t",
    quote = F, row.names = F
)
# read back in table
df_T <- read.table(
    file = "Tcell_interactions/Tcellsource_interactionstrengths_reps_OT.txt",
    sep = "\t", header = TRUE
)
df_T <- read.table(
    file = "Fibroblast_interactions/Fibroblastssource_interactionstrengths_reps_OT.txt",
    sep = "\t", header = TRUE
)
...
for (k in 1:length(targets)) {
    ...
    # write
    write.csv(emm_df,
        file = paste0(outdir, sender, "-", target1, "_emm.csv"),
        row.names = F
    )
    # get contrast means
    ...
    # write df
    write.csv(pw_df,
        file = paste0(outdir, sender, "-", target1, "_interactions_con.csv"),
        row.names = F
    )
    ...
    # write df
    write.csv(con_df,
        file = paste0(outdir, sender, "-", target1, "_contrasts_R-NR.csv"),
        row.names = F
    )
    # line plot
...
# write
write.table(df_T,
    file = "Tcelltarget_interactionstrengths_OT_reps.txt", sep = "\t",
    quote = F, row.names = F
)
write.table(df_T,
    file = "Fibroblaststarget_interactionstrengths_OT_reps.txt", sep = "\t",
    quote = F, row.names = F
)
# read back in
df_T <- read.table(
    file = "Tcell_interactions/Tcelltarget_interactionstrengths_OT_reps.txt",
    sep = "\t", header = TRUE
)
df_T <- read.table(
    file = "Fibroblast_interactions/Fibroblaststarget_interactionstrengths_OT_reps.txt",
    sep = "\t", header = TRUE
)
# set target
target <- "T_Cells_and_Neutrophils"
...
for (k in 1:length(sources)) {
    ...
    my_title <- paste0(
        "Celltype communication source: ", source1,
        " target: ", target
    )
    ...
    # write
    write.csv(emm_df,
        file = paste0(outdir, source1, "-", target, "_emm.csv"),
        row.names = F
    )
    ...
    # write df
    write.csv(pw_df,
        file = paste0(outdir, source1, "-", target, "_interactions_con.csv"),
        row.names = F
    )
    ...
    # write df
    write.csv(con_df,
        file = paste0(outdir, source1, "-", target, "_contrasts_R-NR.csv"),
        row.names = F
    )
    ...
    pdf(
        file = paste0(
            outdir, "cc_interaction_strength_emmeans_", source1,
            "-", target, "_noCI.pdf"
        ),
        height = 4, width = 5
    )
