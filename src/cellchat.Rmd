...
```
```{r load_data}
data_dir <- "/w5home/bmoore/Pierre_sc_zebrafish/mouse_objects/"
setwd(data_dir)
filename <- "seurat_mouse_annot_2WkNR_14.rds"
name <- basename(filename)
timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
output <- paste0("/w5home/bmoore/Pierre_sc_zebrafish/cellchat2/", name, "_cellchat_allpaths", timestamp)
print(output)
...
```{r subset}
# get gene list
AG_gene_file <- read.csv("/w5home/bmoore/Pierre_sc_zebrafish/zfin_search_results_axonguidegenes.csv",
        header == TRUE,
        sep = ","
)
AG_gene_list <- AG_gene_file$name
# Subset on the expression level of a gene/feature
seurat_obj <- subset(x = seurat_obj, subset = MS4A1 > 2.5)
```
...
ccdb_df <- as.data.frame(CellChatDB$interaction)
write.table(ccdb_df, file = paste0(output, "cellchat_ccdb_df.txt"), sep = "\t", quote = FALSE)
```
# subset DB
```{r subset_db}
...
# get all pathways
df.net.p.all <- reshape2::melt(cellchat@netP$prob, value.name = "prob"); colnames(df.net.p.all)[1:3] <- c("source","target","pathway_name")
# extract dfs # this step gets sig intercations/paths
...
for (i in 1:length(pathways.show.all)) {
        ...
        ggsave(
                filename = paste0(path_dir, pathways.show.all[i], "_path_L-R_contribution.pdf"),
        ...
        netVisual_aggregate(cellchat,
                signaling = pathways.show.all[i], layout = "circle",
        ...
        p4 <- netVisual_bubble(cellchat,
                sources.use = c(6), targets.use = c(4, 12),
        ...
        p5 <- netVisual_bubble(cellchat,
                sources.use = c(8), targets.use = c(6, 5, 3, 7, 13, 14),
                signaling = pathways.show.all[i], remove.isolate = FALSE
        )
...
ng <- netAnalysis_signalingRole_network(cellchat,
        signaling = pathways.show, width = 8,
...
for (i in 1:length(reps)) {
        ...
        qs_save(seurat_sub, paste0(filename, "_", rep, ".qs2"))
        qs_to_rds(input_file = paste0(filename, "_", rep, ".qs2"), output_file = paste0(filename, "_", rep, ".rds"))
