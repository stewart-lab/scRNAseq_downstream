...
# split layers
```{r split_layers}
merged_seurat[["RNA5"]] <- split(merged_seurat[["RNA5"]], f = merged_seurat$orig.ident)
merged_seurat
...
if(integration_type == "cca"){
  ...
  p1 <- DimPlot(merged_seurat,reduction = "umap.cca",
    group.by = c("orig.ident", "CellType1", cluster_name),
...
```{r get_cells}
new.obj <- GetAssayData(object = merged_seurat, layer = "data")[c("HES7","TBX6"),]>0
```
```{r subset}
#create cell names as metadata colum
#merged_seurat[["CellName"]] <- colnames(merged_seurat)
...
merged_seurat_subset <- subset(merged_seurat, 
                            cells = cellstokeep)
merged_seurat_subset
```
```{r plot}
p1 <- DimPlot(merged_seurat_subset, reduction = "umap.cca", label = TRUE, 
        pt.size = 0.5, group.by = "CellType1")
p2 <- DimPlot(merged_seurat, reduction = "umap.cca", label = TRUE, 
        pt.size = 0.5, group.by = "CellType1")
p3 <- DimPlot(merged_seurat_subset, reduction = "umap.cca", 
        pt.size = 0.5, group.by = "orig.ident")
p4 <- DimPlot(merged_seurat_subset, reduction = "umap.cca", label = TRUE, 
        pt.size = 0.5, group.by = "cca_clusters")
p5 <- FeaturePlot(merged_seurat_subset, features = c("HES7","TBX6"), ncol = 2,
                           pt.size = 0.5, reduction = "umap.cca") &
                           scale_color_viridis(option="B")
combined_plot <- ((p1 | p2) / (p3 | p4) / p5) + plot_layout(width = c(2, 2, 2),
                                                      heights = c(1, 1, 1))
pdf(file = paste0(output,"umap_cca_subset_HES7-TBX62.pdf"), width = 8, height = 8)
    print(combined_plot)
dev.off()
```