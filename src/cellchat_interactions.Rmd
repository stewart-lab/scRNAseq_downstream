...
data.dir <- paste0("./comparison_6wk_clusters_0.75", timestamp)
...
```{r}
name <- "6wk_R_NR"
cellchat.NR <- readRDS("/w5home/bmoore/Pierre_sc_zebrafish/cellchat_0.75_clusters/seurat_mouse_annot_6WkNR_cellchat_20260625_103123/seurat_mouse_annot_6WkNR_cellchat_obj.rds")
cellchat.R <- readRDS("/w5home/bmoore/Pierre_sc_zebrafish/cellchat_0.75_clusters/seurat_mouse_annot_6WkR_cellchat_20260625_103119/seurat_mouse_annot_6WkR_cellchat_obj.rds")
...
for (i in 1:length(object.list)) {
        netVisual_chord_gene(object.list[[i]],
                sources.use = fibroblast, targets.use = not_fibroblast,
                lab.cex = 0.1, title.name = paste0("Signaling from Fibroblast - ", names(object.list)[i])
        )
}
dev.off()
...
for (i in 1:length(object.list)) {
        netVisual_chord_gene(object.list[[i]],
                sources.use = not_fibroblast, targets.use = fibroblast,
                lab.cex = 0.1, title.name = paste0("Signaling to Fibroblast - ", names(object.list)[i])
        )
}
dev.off()
```
...
```{r load_data}
# load(paste0("cellchat_object.list_", name, ".RData"))
# load(paste0("cellchat_merge_", name, ".RData"))
```
