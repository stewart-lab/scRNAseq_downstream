...
if (DO_RUN_CASSIA) {
  runCASSIA_pipeline(
    ...
    # provider/model/credentials as every other stage avoids that failure
    # path; the sys.stdout patch above covers any print() call that still
    # happens (e.g. on a genuine transient error) so it no longer deadlocks
    # either way.
...
if (!file.exists(cassia_results_path)) {
  ...
  # do_merge_annotations was enabled -- the plot_if_annotated chunk below
  # was written against the raw columns and found nothing to plot.
...
# (Cassia_clus1_merged_grouping_1/2/3) when do_merge_annotations produced
# them -- they're CASSIA's own consolidated broad/detailed/very-detailed
# labels, shorter and cleaner than the raw per-cluster predictions, which
# is the whole point of running the merge step. Fall back to the raw
# columns otherwise (e.g. if merging was disabled or failed). Deliberately
# excludes from the raw-column fallback:
...
merged_cols <- intersect(
  c("Cassia_clus1_merged_grouping_1", "Cassia_clus1_merged_grouping_2", "Cassia_clus1_merged_grouping_3"),
