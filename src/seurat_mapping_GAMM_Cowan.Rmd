...
METADATA_REF <- "/w5home/bmoore/Gamm_scRNAseq/data/cowan_metadata/meta_30weeks_mod.txt"
METADATA_GAMM <- "/w5home/bmoore/Gamm_scRNAseq/data/gamm_metadata/gamm_manual_annot_metadata_c0.5.txt"
...
file.copy("/w5home/bmoore/Gamm_scRNAseq/config.json", file.path(output, "config.json"))
config <- fromJSON(file.path(output, "config.json"))
output <- paste0(output, "/")
source("/w5home/bmoore/Gamm_scRNAseq/src/sc_pipeline_functions.R")
```
...
```{r metadata}
# read in meta.data
# metadata is in the data folder of the github repo
metadata <- read.csv2(METADATA_REF, sep="\t", header=TRUE, row.names=1)
# subset by D205 data
...
print(p1)
dev.off()
p2 <- VlnPlot(human_IMR90.seurat, features = "percent.mt", split.by = "cell_type3")
...
metadata.gamm <- read.csv(METADATA_GAMM, 
...
table(gamms2.1@meta.data$CellType_manual)
```
...
print(pg)
dev.off()
pc <- DimPlot(gamms2.1, reduction = "umap", group.by = "CellType_manual", label = TRUE,
...
print(pg)
dev.off()
pc <- DimPlot(gamms2.2, reduction = "umap", group.by = "CellType_manual", label = TRUE,
              label.size = 3, repel = TRUE) + ggtitle("Query manually annotated labels")
...
print(p1 + p2)
dev.off()
p3 <- DimPlot(gamms2.1, reduction = "ref.umap", group.by = "CellType_manual", label = TRUE,
...
print(p1 + p2)
dev.off()
p3 <- DimPlot(gamms2.2, reduction = "ref.umap", group.by = "CellType_manual", label = TRUE,
              label.size = 3, repel = TRUE) + ggtitle("Manually annotated labels")
