...
scRNAseqData_scaled <- if (seurat_package_v5) {
  as.matrix(seurat.obj[["RNA"]]$scale.data) 
} else {
  as.matrix(seurat.obj[["RNA3"]]@scale.data)
}
...
cL_resutls <- do.call("rbind", lapply(unique(seurat.obj@meta.data$seurat_clusters), 
            function(cl){es.max.cl = sort(rowSums(es.max[ ,
            rownames(seurat.obj@meta.data[seurat.obj@meta.data$seurat_clusters==cl, ])]), 
            decreasing = !0) head(data.frame(cluster = cl, type = names(es.max.cl), 
            scores = es.max.cl, ncells = sum(seurat.obj@meta.data$seurat_clusters==cl)), 10)
}))
