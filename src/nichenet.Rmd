...
# read in object 1
seuratObjNR <- readRDS(file = paste0(projPath, "seurat_mouse_annot_6WkNR.rds"))
seuratObjR <- readRDS(file = paste0(projPath, "seurat_mouse_annot_6WkR.rds"))
...
network_path <- "/w5home/bmoore/Pierre_sc_zebrafish/nichenet/nichenet_networks/"
...
output <- paste0("/w5home/bmoore/Pierre_sc_zebrafish/nichenet/nichnet_", name, timestamp)
print(output)
...
pdf(file = paste0(output, "seurat_mouse_umap.pdf"), height = 6, width = 10)
patchwork::wrap_plots(plots = list(gg1, gg2))
dev.off()
```
```{r}
# define reference condition and condition of interest
condition_oi <- "6Wk-R" # this is the "case" cond
condition_reference <- "6Wk-NR" # this is the "control"
```
```{r}
...
# define receiver
receivers <- c("T_Cells_and_Neutrophils", "Fibroblasts") # , "T_Cells_and_Neutrophils", "Immature_Neurons","Gabaergic_Neurons","Glutamatergic_Neurons",,"Glycinergic_Neurons", "Motor_Neurons"
...
for (i in 1:length(receivers)) {
  ...
  if (length(geneset_oi) == 0) {
    geneset_oi <- DE_table_receiver[DE_table_receiver$p_val_adj <= 0.06 &
      abs(DE_table_receiver$avg_log2FC) >= 0.2, ]
    geneset_oi <- rownames(geneset_oi)[rownames(geneset_oi) %in%
      rownames(ligand_target_matrix)]
  ...
  best_upstream_ligands <- append(best_upstream_ligands, "App")
