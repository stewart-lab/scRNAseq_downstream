...
# read in object 1
seuratObjNR <- readRDS(file= paste0(projPath,"seurat_mouse_annot_2WkNR.rds"))
seuratObjR <- readRDS(file= paste0(projPath,"seurat_mouse_annot_2WkR.rds"))
print(seuratObjNR@meta.data %>% head())
print(seuratObjR@meta.data %>% head())
```
```{r}
# Merge two Seurat objects
seuratObj <- merge(x = seuratObjNR, y = seuratObjR, merge.data = TRUE)
...
pdf(file=paste0(output,"seurat_mouse_umap2wk.pdf"), height=6, width=10)
patchwork::wrap_plots(plots = list(gg1,gg2))
dev.off()
```
```{r}
# define reference condition and condition of interest
condition_oi <- "2Wk-R" # this is the "case" cond
condition_reference <- "2Wk-NR" # this is the "control"
```
```{r}
...
# define receiver
receivers <- c("Fibroblasts") # , "T_Cells_and_Neutrophils"
...
lig_activity_heatmap <- function(vis_ligand_aupr, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand_activity_heatmap.pdf"), width=3, height=8)
  print(make_heatmap_ggplot(vis_ligand_aupr, y_name = "Prioritized ligands", 
...
lig_target_heatmap <- function(vis_ligand_target, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand-target_reg_potential_heatmap.pdf"), width=8, height=5)
...
LR_heatmap <- function(vis_ligand_receptor_network, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand-receptor_interactions_heatmap.pdf"))
  print(make_heatmap_ggplot(t(vis_ligand_receptor_network), y_name = "Prioritized ligands", 
...
ligand_lfc_heatmap <- function(vis_ligand_lfc, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand_logfc_heatmap.pdf"))
  print(make_threecolor_heatmap_ggplot(vis_ligand_lfc, y_name = "Prioritized ligands", 
...
ligand_dotplot <- function(seuratObj, best_upstream_ligands, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand_expr_dotplot.pdf"))
...
ligand_lineplot <- function(ligand_activities_all, potential_ligands_focused, output, name, receiver){
  pdf(file=paste0(output, name, "_", receiver, "_ligand_activities_lineplot.pdf"))
  print(make_line_plot(ligand_activities = ligand_activities_all, 
...
chord_diagram <- function(sender_celltypes, circos_links, plot_name, output, name, receiver){
  ...
  # Assign colors 
  sender_celltypes_gen <- append(sender_celltypes, "General")
  ...
  pdf(file=paste0(output, name, "_", receiver, "_", plot_name, "_chord.pdf"))
    print(cowplot::plot_grid(make_circos_plot(vis_circos_obj, transparency = TRUE, 
                    link.visible = TRUE, args.circos.text = list(cex = 0.5)), 
                    circos_legend_grob, rel_widths = c(1, 0.1))) # combine chord plot and legend
  dev.off()
}

mushroom_plot <- function(prioritized_table, output, name, receiver){
  mp <- make_mushroom_plot(prioritized_table, top_n = 30, show_all_datapoints = TRUE, show_rankings = TRUE)
  pdf(file=paste0(output, name, "_", receiver, "_ligand-prioritize_mushroom_plot.pdf"))
    print(mp + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5), 
...
sig_network_maps <- function(order_ligands, order_targets, weighted_networks, 
  ...
  for (i in 1:length(ligands_oi)){
    ligand <- ligands_oi[i]
    print(ligand)
    active_signaling_network_ligand <- get_ligand_signaling_path(ligands_all = ligand, 
      targets_all = targets_oi, weighted_networks = weighted_networks,
      ligand_tf_matrix = ligand_tf_matrix, top_n_regulators = 4, minmax_scaling = TRUE)
    signaling_graph_lig <- diagrammer_format_signaling_graph(signaling_graph_list = active_signaling_network_ligand, 
...
for (i in 1:length(receivers)){
  ...
  suppressWarnings({ligand_activities <- predict_ligand_activities(geneset = geneset_oi, 
  ...
  write.table(ligand_activities_df,file=paste0(output, name, "_", receiver,"_ligand_activities.txt"), 
        sep="\t", quote=FALSE)
  ...
  write.table(as.data.frame(active_ligand_target_links_df), file=paste0(output, name, "_", receiver,"_ligand-target_links.txt"), 
        sep="\t", quote=FALSE)
  ...
  # write out
  write.table(as.data.frame(ligand_receptor_links_df), file=paste0(output, name, "_", receiver,"_ligand-receptor_links.txt"), 
        sep="\t", quote=FALSE)
  ...
  active_ligand_target_links <- prepare_ligand_target_visualization(ligand_target_df = active_ligand_target_links_df, 
                                ligand_target_matrix = ligand_target_matrix, cutoff = 0.05) # 0.25
  if (all(is.na(active_ligand_target_links))==FALSE){
    ...
    # write out
    write.table(as.data.frame(vis_ligand_target), file=paste0(output, name, 
      "_", receiver,"_ligand-target_reg_potential.txt"), sep="\t", quote=FALSE)
    # heatmap
    ...
    # chord diagram
    chord_diagram(sender_celltypes, circos_links, "ligand-target_interactions", 
                output, name, receiver)
    # make signaliing maps between ligand and targets
    print("Making signaling network maps")
    sig_network_maps(order_ligands, order_targets, weighted_networks, 
                      ligand_tf_matrix, output, name, receiver)
  } else {
  ...
  DE_table_top_ligands <- lapply(celltype_order[celltype_order %in% sender_celltypes], 
                        get_lfc_celltype, seurat_obj = seuratObj, condition_colname = "Type", 
  ...
  # chord plot
  chord_diagram(sender_celltypes, lr_network_top_df, "ligand-receptor_interactions", 
                output, name, receiver)
  ...
  info_tables <- generate_info_tables(seuratObj, celltype_colname = 
  ...
  prioritized_table <- generate_prioritization_tables(sender_receiver_info = 
  ...
  # write out
  write.table(as.data.frame(prioritized_table), file=paste0(output, name, 
    "_", receiver,"_ligand-receptor_prioritized_table.txt"), sep="\t", quote=FALSE)
...
predictions_list <- lapply(1:n, assess_rf_class_probabilities, folds = k, 
...
# write out
write.table(as.data.frame(top_predicted_genes), file=paste0(output, name, 
    "_", receiver,"_top_predicted_target_genes.txt"), sep="\t", quote=FALSE)
```
...
for (i in 1:length(ligands_oi)){
  ...
  active_signaling_network_ligand <- get_ligand_signaling_path(ligands_all = ligand, 
    targets_all = targets_oi, weighted_networks = weighted_networks,
    ligand_tf_matrix = ligand_tf_matrix, top_n_regulators = 4, minmax_scaling = TRUE)
  signaling_graph <- diagrammer_format_signaling_graph(signaling_graph_list = active_signaling_network_ligand, 
    ligands_all = ligand, targets_all = targets_oi, sig_color = "indianred", gr_color = "steelblue")
  signaling_graph %>%
  export_graph(
    file_name = paste0(output, name, "_", receiver,  "_", ligand, "_signaling_graph.pdf"),
