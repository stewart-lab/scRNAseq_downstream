...
setwd(paste0(BETHS_GAMM_DATA_DIR, "cell_composition/"))
...
```{r read_in}
seurat.obj.1 <- readRDS(file = "../GAMM_S1/output_20230921_142919/GAMM_S1_clabeled-clusters_0.5.rds") # reference
seurat.obj.2 <- readRDS(file = "../GAMM_S2/output_20230830_155530/GAMM_S2_clabeled-clusters_0.5.rds") # query
# reference cell type column
annot_ref<- seurat.obj.1$CellType_manual
```
...
if (CROSS_SPECIES=="yes"){
  ...
  seurat.obj.2 <- RenameIdents(object = seurat.obj.2, `output_S2-2_mm_mt_GeneFull` = "pig", `gamm_S2` = "pig", `output_S2-1_mm_mt_GeneFull` = "pig")
  seurat.obj.2[["orig.ident"]] <- Idents(object = seurat.obj.2)
  Idents(object = seurat.obj.1) <- "orig.ident"
}
table(seurat.obj.2$orig.ident)
table(seurat.obj.1$orig.ident)

#saveRDS(seurat.obj.2, file =  paste0(output,"gamms2.2_cca_pred.rds"))
...
if (CROSS_SPECIES=="yes"){
    ...
    if (COMP_TYPE=="scpred"){
       ...
       table(seurat.obj.2$predicted.id)
        ...
        table(seurat.obj.1@meta.data$CellType_manual)
        table(seurat.obj.2@meta.data$CellType_manual)
    } else {
...
```{r add_meta}
if (COMP_TYPE=="manual"){
    seurat.obj.1 <- AddMetaData(seurat.obj.1, metadata = seurat.obj.1@meta.data$CellType_manual, col.name= "CellType")
    table(seurat.obj.1$CellType)
    seurat.obj.2 <- AddMetaData(seurat.obj.2, metadata = seurat.obj.2@meta.data$CellType_manual, col.name= "CellType")
